...
bool DS3231::isAvailable(){
    auto reg = readReg(0x0F); // Read Status Register
    return !(reg & 0x80);   // Read OSF Bit(MSB)
}

void DS3231::writeTimeToRTC(time_t t){
    struct tm local;
    localtime_r(&t, &local);
    uint8_t buf[7] = {0};
    ...
    buf[5] = dec2bcd(local.tm_mon + 1) & 0x1F; // Ignore Century bit(MSB)
    buf[6] = dec2bcd(local.tm_year % 100) & 0x3F;    
    writeRegBurst(0, buf, 7);
    writeReg(0x0F, readReg(0x0F) & ~0x80);  // Clear OSF Bit (MSB)
}

time_t DS3231::readTimeFromRTC(){
